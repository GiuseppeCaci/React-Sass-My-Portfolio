...
import LoadingBalls from "../components/ChatBoxComponent/LoadingBalls";
...
const ContattiChatBox = () => {
  ...
  const handleUseMessage = (question) => {
    ...
    setMessages((prevMessages) => [
      ...prevMessages,
      { sender: "bot", text: ["loading"], buttons: [], textButtons: [] },
    ]);
    setIsLoading(true);
  ...
  const generateBotResponse = (question) => {
    ...
    switch (question) {
      case t("responses.webAppCreation.question"):
        ...
        textButtonQuestion = [
          t("responses.notListenMusicFunky.question"),
          t("responses.favoriteQuote.question"),
        ];
        break;
      ...
      case t("responses.contactMe.question"):
        ...
        buttons = [
          t("buttons.contactMe"),
          `mailto:${import.meta.env.VITE_EMAIL}`,
        ];
        break;
      ...
      default:
        response = [
          t("messages.defaultResponse") || "Non ho capito, puoi ripetere?",
        ];
    }
    return {
      sender: "bot",
  ...
  const Message = memo(({ msg, theme, handleUseMessage }) => {
    ...
    return (
      <div
        ...
        } ${msg.text[0] === "loading" ? "pc-write" : null} message ${
        ...
        <div className="button-chat">
          ...
          {msg.textButtons.length === 0 &&
            ...
            !isLoading && (
              <button
                onClick={() =>
                  handleUseMessage(t("messages.buttonBackToQuestions"))
                }
              >
                {t("cta-1")}
              </button>
  ...
  return (
    ...
    >
      ...
      <div className="more-margin-bottom"></div>
    </div>
  );
};
